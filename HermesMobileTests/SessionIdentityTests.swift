...
final class SectionVisibilitySettingsTests: XCTestCase {
    ...
    func testHidingOneSectionLeavesTheOthersVisible() {
        defaults.set(false, forKey: SectionVisibilitySettings.skillsKey)

        XCTAssertFalse(SectionVisibilitySettings.isVisible(SectionVisibilitySettings.skillsKey, in: defaults))
        for key in allKeys where key != SectionVisibilitySettings.skillsKey {
